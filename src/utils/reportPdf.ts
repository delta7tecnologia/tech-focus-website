...
function buildHtml(r: ReportData): string {
  const photos = r.photos
    .map((p) => {
      if (p.external) {
        return `
        <div class="pdf-avoid-break" style="break-inside: avoid; page-break-inside: avoid; width: 48%; margin-bottom: 16px;">
...
<div style="width: 794px; padding: 40px 48px; font-family: 'Helvetica', Arial, sans-serif; color: #1e293b; background: white; box-sizing: border-box;">
  ...
  <div id="report-footer-block" style="margin-top: 32px; padding-top: 12px; border-top: 2px solid #1e3a8a; break-inside: avoid; page-break-inside: avoid; -webkit-column-break-inside: avoid;">
    <div style="display: flex; gap: 16px; align-items: flex-start;">
      ...
      <div style="flex: 1; min-width: 0;">
        ...
        ${r.validationUrl ? `<div style="font-size: 8px; color: #475569; margin-top: 6px; word-break: break-all;"><strong>Link de validação:</strong> ${r.validationUrl}</div>` : ''}
        <div style="font-size: 9px; color: #94a3b8; margin-top: 8px; font-style: italic;">
          Documento com integridade garantida pela plataforma Delta7. Verifique pelo QR Code ou link acima.
        </div>
      </div>
    </div>
  </div>
