...
const ProposalSignatureLinksManager: React.FC<Props> = ({ proposalId }) => {
  const { user } = useAuth();
  ...
  const { data: links = [], isLoading } = useQuery({
    queryKey: ['proposal-signature-links', proposalId],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await (supabase as any).from('proposal_signature_links').insert({
    ...
    onSuccess: (token) => {
      const url = `${window.location.origin}/assinar-proposta/${token}`;
      navigator.clipboard?.writeText(url).catch(() => {});
      toast({ title: 'Link gerado e copiado!', description: 'Cole no WhatsApp ou e-mail do signatário.' });
      setSignerName('');
      setSignerEmail('');
      qc.invalidateQueries({ queryKey: ['proposal-signature-links', proposalId] });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from('proposal_signature_links').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const copyLink = (token: string) => {
    navigator.clipboard?.writeText(`${window.location.origin}/assinar-proposta/${token}`);
    ...
    <Card className="border-blue-200">
      <CardContent className="p-6 space-y-4">
        ...
        <p className="text-xs text-gray-600">
          Gere um link único (válido por 7 dias) para o cliente aceitar a proposta sem precisar fazer login. Copie e envie via WhatsApp ou e-mail.
        </p>
...
export default ProposalSignatureLinksManager;
