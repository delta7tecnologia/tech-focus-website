...
const ItSupportProposals: React.FC = () => {
  const { user } = useAuth();
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from('it_support_proposals').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-gray-900">Propostas — Suporte de TI Mensal</h3>
          <p className="text-sm text-gray-500">Contratos de suporte técnico mensal para empresas.</p>
        </div>
                ...
                <CardContent className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      Mensal: <strong>{formatBRL(Number(p.monthly_total) || 0)}</strong> · Setup: {formatBRL(Number(p.setup_fee) || 0)} · {p.contract_months} meses ·{' '}
      ...
      <Dialog open={isFormOpen} onOpenChange={(o) => !o && closeForm()}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? `Editar Proposta — ${editing.proposal_number}` : 'Nova Proposta de Suporte de TI'}</DialogTitle>
          </DialogHeader>
          <ItSupportProposalForm proposal={editing} onClose={closeForm} />
        </DialogContent>
      </Dialog>
    </div>
  );
...
export default ItSupportProposals;
