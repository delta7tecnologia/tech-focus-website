...
const ItSupportProposalSignatureLinksManager: React.FC<Props> = ({ proposalId }) => {
  const { user } = useAuth();
  const { toast } = useToast();
  const qc = useQueryClient();
  const [signerName, setSignerName] = useState('');
  const [signerEmail, setSignerEmail] = useState('');
  const [signerRole, setSignerRole] = useState<'cliente' | 'responsavel'>('cliente');

  const { data: links = [], isLoading } = useQuery({
    queryKey: ['it-support-proposal-signature-links', proposalId],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await (supabase as any).from('it_support_proposal_signature_links').insert({
    ...
    onSuccess: (token) => {
      const url = `${window.location.origin}/assinar-suporte/${token}`;
      navigator.clipboard?.writeText(url).catch(() => {});
      toast({ title: 'Link gerado e copiado!', description: 'Cole no WhatsApp ou e-mail do signatário.' });
      setSignerName('');
      setSignerEmail('');
      qc.invalidateQueries({ queryKey: ['it-support-proposal-signature-links', proposalId] });
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await (supabase as any).from('it_support_proposal_signature_links').delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const copyLink = (token: string) => {
    navigator.clipboard?.writeText(`${window.location.origin}/assinar-suporte/${token}`);
    ...
    <Card className="border-blue-200">
      <CardContent className="p-6 space-y-4">
        ...
        <p className="text-xs text-gray-600">
          Gere um link único (válido por 7 dias) para o cliente aceitar o contrato sem precisar fazer login.
        </p>
...
export default ItSupportProposalSignatureLinksManager;
