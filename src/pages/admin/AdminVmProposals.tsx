...
import { downloadVmProposalPdf } from '@/utils/vmProposalPdf';
...
const AdminVmProposals: React.FC = () => {
  const { user } = useAuth();
  ...
  const { data: proposals, isLoading } = useQuery({
    queryKey: ['vm-proposals'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('vm_proposals' as any)
        .select('*')
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('vm_proposals' as any).delete().eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['vm-proposals'] });
  ...
  const download = async (p: any) => {
    ...
    try {
      await downloadVmProposalPdf({
        ...
        integrityHash: p.integrity_hash || 'sem-hash'.padEnd(64, '0'),
      });
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Propostas Locação de VMs</h2>
        <p className="text-gray-500">Gere propostas de locação de máquinas virtuais gerenciadas.</p>
      </div>

      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">{proposals?.length || 0} proposta(s)</p>
        <Button onClick={() => { setEditing(null); setOpen(true); }} className="bg-blue-900 hover:bg-blue-800">
          <FilePlus className="w-4 h-4 mr-2" /> Nova Proposta VM
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid gap-3">
          {proposals.map((p) => {
            ...
            const menor = (p.planos || []).reduce((m: number, x: any) => (m === 0 || x.mensal < m ? x.mensal : m), 0);
            return (
              <Card key={p.id}>
                <CardContent className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {(p.vms || []).length} VM(s) · A partir de <strong>{formatBRL(menor)}</strong>/mês ·{' '}
                      {new Date(p.created_at).toLocaleDateString('pt-BR')}
                    </p>
                  </div>
                  <div className="flex gap-1 flex-shrink-0">
                    {p.integrity_hash && (
                      <Button size="icon" variant="ghost" onClick={() => download(p)} disabled={downloading === p.id} title="Baixar PDF">
                    ...
                    {p.created_by === user?.id && (
                      <Button size="icon" variant="ghost" className="text-red-600" onClick={() => {
                        if (confirm('Excluir esta proposta?')) del.mutate(p.id);
                      }} title="Excluir">
      ...
      <Dialog open={open} onOpenChange={(o) => { if (!o) { setOpen(false); setEditing(null); } }}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? `Editar — ${editing.proposal_number}` : 'Nova Proposta de Locação de VMs'}</DialogTitle>
          </DialogHeader>
          <VmProposalForm proposal={editing} onClose={() => { setOpen(false); setEditing(null); }} />
        </DialogContent>
      </Dialog>
    </div>
  );
...
export default AdminVmProposals;
