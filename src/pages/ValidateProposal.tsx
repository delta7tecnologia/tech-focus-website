...
const ValidateProposal = () => {
  useSEO({ title: 'Validar proposta comercial | Delta7 Tecnologia', description: 'Valide a autenticidade de uma proposta comercial emitida pela Delta7 Tecnologia.', noindex: true });
  ...
  const { data: p, isLoading, error } = useQuery({
    queryKey: ['validate-proposal', hash],
    queryFn: async () => {
      ...
      const { data: rows, error } = await (supabase as any).rpc('get_commercial_proposal_by_hash', { p_hash: cleanHash });
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            <h2 className="text-lg font-bold text-blue-900">Proposta {p.proposal_number}</h2>
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <div><dt className="text-xs text-gray-500 uppercase">Custo mensal</dt><dd className="font-medium">{formatBRL(Number(p.monthly_total) || 0)}</dd></div>
              <div><dt className="text-xs text-gray-500 uppercase">Ativação</dt><dd className="font-medium">{formatBRL(Number(p.activation_fee) || 0)}</dd></div>
              <div><dt className="text-xs text-gray-500 uppercase">Emitida em</dt><dd className="font-medium">{fmt(p.generated_at)}</dd></div>
              <div><dt className="text-xs text-gray-500 uppercase">Validade</dt><dd className="font-medium">{p.validity_days} dias</dd></div>
            </dl>
